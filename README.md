...
By default `serve` binds to `127.0.0.1` (localhost only) and sets CORS to `*`. Use `--host 0.0.0.0` to expose it on the network and `--cors <origin>` to restrict cross-origin access when doing so.
